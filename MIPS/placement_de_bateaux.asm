...
placeBateaux:			#ceci est la fonction "main" du placement de bateaux, $a0 = adresse grille
	addiu $sp, $sp, -12	#on veut stocker t3 donc on décremente de 3 mots
	sw $ra, ($sp)		#PRO
	sw $fp, 4($sp)		#PRO
	addiu $fp, $sp, 12	#PRO
	...
	sw $t3, 8($sp)		#on stock t3 dans la pile car on veut pas perdre sa valeure
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! changer avec random
	ori $a0, $0, 0		#$a0 = point de départ du bateau
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! changer avec random
	ori $a1, $0, 1		#$a1 = orientation du bateau
while_for_placeBateaux:		#on va controler si le bateau peut être placé, si non augmente le point0 de 1 j'usqu'à pouvoir placer le bateau
	...
	lw $t3, 8($sp)		#on reprend la valeur de $t3 = COLS*LIGS - 1
...
while_for_placeBateaux3:		#on va controler si le bateau peut être placé, si non augmente le point0 de 1 j'usqu'à pouvoir placer le bateau
	jal controleDePlacement
	lw $t3, 8($sp)			#on reprend $t3 = COLS*LIGS - 1
